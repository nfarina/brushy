...
/// context: no filesystem, no network, no DOM — only `brushy`, `doc`,
...
enum ScriptHost {
    ...
    static func run(code: String, session: ScriptSession, timeout: TimeInterval = 20) -> Outcome {
        ...
        context.setObject(host, forKeyedSubscript: "__brushyHost" as NSString)

        context.evaluateScript(ScriptPrelude.source, withSourceURL: URL(string: "brushy-prelude.js"))
        ...
        let wrapped = "__brushyRun(function(){" + code + "\n})"
