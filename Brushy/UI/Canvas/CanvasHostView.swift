...
final class CanvasHostView: NSView {
    ...
    /// `BrushyDocument` validates them exactly as it does in the main menu.
    override func menu(for event: NSEvent) -> NSMenu? {
        guard store.textSession == nil, !store.selection.isEmpty else { return nil }
        let menu = NSMenu()
        menu.addItem(withTitle: "Layer via Copy",
                     action: #selector(BrushyDocument.layerViaCopy(_:)), keyEquivalent: "")
        menu.addItem(withTitle: "Layer via Cut",
                     action: #selector(BrushyDocument.layerViaCut(_:)), keyEquivalent: "")
        menu.addItem(.separator())
        menu.addItem(withTitle: "Select Inverse",
                     action: #selector(BrushyDocument.invertSelection(_:)), keyEquivalent: "")
        menu.addItem(withTitle: "Deselect",
                     action: #selector(BrushyDocument.deselect(_:)), keyEquivalent: "")
        menu.addItem(withTitle: "Transform Selection",
                     action: #selector(BrushyDocument.transformSelection(_:)), keyEquivalent: "")
        menu.addItem(.separator())
        menu.addItem(withTitle: "Fill…",
                     action: #selector(BrushyDocument.showFill(_:)), keyEquivalent: "")
        menu.addItem(withTitle: "Crop to Selection",
                     action: #selector(BrushyDocument.cropToSelection(_:)), keyEquivalent: "")
        return menu
    }
    ...
    // Select > All is a BrushyDocument action (reached through the
