...
enum ChatPrompt {
    static func systemInstruction() -> String {
        """
        You are the assistant built into Brushy, a small macOS image editor with layers (think a lean Photoshop). \
    ...
    static func contextPrefix(registry: DocumentRegistry) -> String {
        ...
        guard !entries.isEmpty else {
            return "[Context]\nNo documents are open. Scripts can create one with brushy.newDocument(width, height)."
        }
