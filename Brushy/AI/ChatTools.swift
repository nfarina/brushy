...
final class ChatTools {
    ...
    static let declarations: [ToolDeclaration] = [
        ToolDeclaration(
            name: executeName,
            description: "Run JavaScript against the open documents using the brushy API (see the system prompt). Changes apply atomically as one undo step per document when the script returns; if it throws, nothing changes. Returns the script's return value, console output, and a summary of what changed.",
            parameters: .object([
                "type": .string("object"),
                "properties": .object([
                    "code": .object(["type": .string("string"),
                                     "description": .string("The JavaScript to run. `doc` is the active document; `brushy.doc(\"doc2\")` reaches others. Use `return` to report a value.")]),
    ...
    private func generateImage(_ call: FunctionCall) async -> ToolOutcome {
        ...
        let code = "brushy.doc(\"\(id)\").addImage(\"generated\", \(options));"
...
enum ChatRenderer {
    static func composite(_ document: Document, maxSide: CGFloat) -> CGImage? {
        ...
        let white = CIColor(red: 1, green: 1, blue: 1, alpha: 1, colorSpace: BrushyColorSpace.sRGB)!
        image = image.composited(over: CIImage(color: white).cropped(to: rect))
        return engine.context.createCGImage(image, from: rect, format: .RGBA8, colorSpace: BrushyColorSpace.sRGB)
    }
