...
// No `@testable import Brushy`: the test target compiles the app sources
...
final class ClipboardTests: XCTestCase {
    ...
    func testLayerRoundTripsThroughPasteboard() throws {
        ...
        let original = try rawRGBA8(source, in: BrushyColorSpace.displayP3)
        let roundTripped = try rawRGBA8(decoded.source, in: BrushyColorSpace.displayP3)
    ...
    func testCopyWithSelectionPastesCroppedLayerInPlace() throws {
        ...
        let raw = try rawRGBA8(pasted.source, in: BrushyColorSpace.displayP3)
